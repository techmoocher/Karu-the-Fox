...
class MusicPlayerWindow(QWidget):
    ...
    def load_config(self):
        ...
        if not config_path.exists() or not self.playlist:
            return
        ...
        try:
            ...
            mode = config.get("playback_mode", "loop_all")
            self.playback_mode = "shuffle" 
            while self.playback_mode != mode:
                self.change_playback_mode()

            last_index = config.get("last_track_index", -1)
            last_position = config.get("last_position", 0)

            if 0 <= last_index < len(self.playlist):
                self.current_index = last_index
                song = self.playlist[last_index]
                ...
                self.song_list_widget.setCurrentRow(last_index)

                self.media_player.durationChanged.connect(
                    lambda d, pos=last_position: self.set_initial_position(pos)
                )
